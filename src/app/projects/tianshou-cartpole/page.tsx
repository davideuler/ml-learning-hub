...
export default function TianshouCartPolePage() {
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16">
      ...
      <div className="card mb-8"><h2 className="font-bold text-[var(--text-primary)] mb-3">Project Background / 项目背景</h2><p className="text-sm text-[var(--text-muted)] leading-relaxed">After learning RL from scratch, the next bottleneck is no longer the Bellman equation, it is engineering repetition. Real teams eventually rely on libraries such as Tianshou to standardize collectors, buffers, and trainers so experiments become faster and more reproducible. <br />当你学过手写 RL 之后，下一个瓶颈通常不再是 Bellman 方程，而是工程重复劳动。真实团队最终会依赖 Tianshou 这样的库，把 collector、buffer 和 trainer 标准化，让实验更快、更稳定、更可复现。</p></div>

      <div className="card mb-8"><h2 className="font-bold text-[var(--text-primary)] mb-3">Problem it solves / 它要解决什么问题</h2><p className="text-sm text-[var(--text-muted)] leading-relaxed">The real problem is not CartPole itself. The problem is how to stop rebuilding RL infrastructure every time you try a new experiment. This project solves that by teaching how framework abstractions map to the concepts you already know, so you gain speed without losing understanding. <br />这个项目要解决的并不是 CartPole 本身，而是“每做一个新实验就重搭一遍 RL 基础设施”的低效问题。它通过解释框架抽象和你已掌握概念之间的映射关系，让你在不失去理解力的前提下获得开发速度。</p></div>
